...
1. **ブログを最初に公開する。** Zennより先でないと検索エンジンにどちらが原典か示せない。
   そしてXのCTAリンクが404になる
2. **Xへの投稿は、ヒーロー画像が公開されてから。** 次項の理由による

前提: `.env` に X の認証情報4つが入っている。無い場合はその時点で止めてユーザーに伝える。

## ヒーロー画像とXの順序（取り返しがつかない）

**Xはリンクカードを投稿した瞬間にキャッシュする。** 後から画像を追加しても、
すでに投稿済みのカードには**遡って反映されない**。貼り直す以外に直す方法がない。

一方、ブログとZennは画像を後から足しても普通に反映される。

したがって、画像が間に合わないときの正しい振る舞いはこうなる。

| 画像の状態 | ブログ | Zenn | Xスレッド |
| --- | --- | --- | --- |
| ある | 公開する | 公開する | 投稿する |
| **まだ無い** | 公開してよい | 公開してよい | **投稿しない。画像を待つ** |

画像が無い場合は、ブログとZennまで公開したうえで「画像を
`public/images/hero/<basename>.<拡張子>` に置いてください。置いたらXに投稿します」と伝えて止まる。
**ここで止まるのは待つ価値があるからで、承認を求めているわけではない。**
...
**投稿前に、この2つが両方200であることを確認する。** 片方でも404なら投稿しない。
