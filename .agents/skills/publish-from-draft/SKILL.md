...
この記事を実際に公開する「続き」は、フルにネットへ出られるこの端末（Codex）で仕上げる。
...
`publish-article`（`.Codex/skills/publish-article/SKILL.md`）を最初からやり直すよう促す。
...
- `.Codex/skills/publish-article/references/publishing.md` — ⑥.5〜⑪の手順とコマンド
- `.Codex/skills/publish-article/references/review.md` — ⑫〜⑬の記録項目と測り方
...
- Coworkが書いた下書きは既にですます調・一人称・コード例なしのはずだが、
  **Xスレッドが常体・絵文字なしになっているかだけは投稿前に自分の目でも確認する**
  （`.Codex/skills/publish-article/references/persona.md`の「AIっぽさが出る典型」参照）
